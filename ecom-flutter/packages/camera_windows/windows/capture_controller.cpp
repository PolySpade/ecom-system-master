...
HRESULT CaptureControllerImpl::FindBaseMediaTypesForSource(
  ...
  // ECOM PATCH: cap the record stream at the resolution preset's height
  // (upstream passes 0xffffffff, so recordings always encode at the
  // camera's maximum resolution - e.g. 4K - regardless of the preset,
  // which overwhelms low-end machines). With the cap, recordings encode
  // at the configured resolution, matching the preview stream.
  if (!FindBestMediaType(
          (DWORD)MF_CAPTURE_ENGINE_PREFERRED_SOURCE_STREAM_FOR_VIDEO_RECORD,
          source, base_capture_media_type_.GetAddressOf(),
          GetMaxPreviewHeight(), nullptr, nullptr)) {
    return E_FAIL;
  }
